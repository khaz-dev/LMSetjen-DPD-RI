...
if (Test-Path "frontend\src\views\auth\Login.jsx") {
    ...
    $loginContent = Get-Content "frontend\src\views\auth\Login.jsx" -Raw
    
    if ($loginContent | Select-String -Pattern "handleGoogleLogin" -Quiet) {
        Check-Pass "handleGoogleLogin function found"
    } else {
        Check-Fail "handleGoogleLogin function not found"
    }
    
    if ($loginContent | Select-String -Pattern "window.google.accounts.id" -Quiet) {
