...
################################################################################
#
#  LMSetjen DPD RI - Deploy on Server
#  Bash Deployment Script for Linux/Ubuntu Servers
#
#  This script runs ON the production/staging server and deploys/updates
...
show_help() {
    ...
    echo ""
    echo "USAGE:"
    echo "  ./deploy-on-staging.sh --mode <mode> [OPTIONS]"
    echo ""
    echo "MODES:"
    ...
    echo "  ./deploy-on-staging.sh --mode update-only"
    echo ""
    echo "  # Full clean rebuild (careful with data!)"
    echo "  ./deploy-on-staging.sh --mode full-clean-build"
    echo ""
    echo "  # Update with database refresh"
    echo "  ./deploy-on-staging.sh --mode update-with-data-refresh --verbose"
    echo ""
    echo "PROJECT PATH:"
    echo "  ${PROJECT_PATH}"
    echo ""
...
sync_environment_file() {
    if [ -f "${PROJECT_PATH}/.env.staging" ]; then
        ...
        print_verbose "✓ Applied .env.staging to .env"
    elif [ ! -f "$ENV_FILE" ]; then
        print_error "Neither .env.staging nor .env found in: $PROJECT_PATH"
        exit 1
    else
        print_warning ".env.staging not found, using existing .env"
    fi
}
...
show_deployment_info() {
    ...
    print_info "  Frontend:  http://localhost:5174 (or http://$(hostname -I | awk '{print $1}'):5174)"
    print_info "  Backend API: http://localhost:8001/api/v1/"
    print_info "  Admin Panel: http://localhost:8001/admin/"
    print_info "  API Docs: http://localhost:8001/api/v1/swagger/"
    print_info ""
