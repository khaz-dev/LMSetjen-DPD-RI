...
/**
 * Q&A Reports Tab
 * ✨ PHASE 7.16: Tab component for managing Q&A and reply reports within Reports page
 * 
 * Similar structure to ReviewAbuseReportsTab but for course Q&A reports
 */
function QAReportsTab() {
    const [reports, setReports] = useState([]);
    const [loading, setLoading] = useState(true);
    const [filterStatus, setFilterStatus] = useState("");
    const [filterType, setFilterType] = useState("question");  // question or message
    ...
    const fetchQAReports = async () => {
        try {
            setLoading(true);
            let url = `/admin/qa-reports/?type=${filterType}`;
            if (filterStatus) {
                url += `&status=${filterStatus}`;
            }
            
            const response = await apiInstance.get(url);
            console.log("[QAReportsTab] API Response:", response.data);
            ...
            console.log(`[QAReportsTab] Found ${data.length} reports`);
            setReports(data);
        } catch (error) {
    ...
    const handleUpdateStatus = async () => {
        ...
        try {
            setUpdating(true);
            const response = await apiInstance.put(`/admin/qa-reports/${selectedReport.id}/?type=${filterType}`, {
                status: detailStatus,
                review_notes: detailNotes,
                reviewed_by: userData.id || userData.user_id
            });
        ...
        <div className="qa-reports-tab">
                ...
                <div className="row">
                    <div className="col-md-4">
                        ...
                        >
                            ...
                            <option value="message">Balasan/Reply</option>
                        </select>
                    </div>
                    <div className="col-md-4">
            ...
            {loading ? (
                ...
                <div className="empty-state text-center py-5">
                    ...
                    <p className="text-muted mt-3">Tidak ada laporan Q&A</p>
                </div>
            ) : (
                        ...
                        <div key={report.id} className="report-card">
                            ...
                            {filterType === "question" && (
                                <div className="report-content-title">
                                    <strong>Pertanyaan:</strong> {report.question_title}
                                </div>
                            )}
                            {filterType === "message" && (
                                <div className="report-content-title">
                                    <strong>Balasan:</strong> {report.message_content?.substring(0, 100)}...
                                </div>
                            )}
                    ...
                    <div className="modal-content detail-modal" onClick={(e) => e.stopPropagation()}>
                        <div className="modal-header">
                            <h5 className="modal-title">
                                ...
                                Detail Laporan {filterType === "question" ? "Pertanyaan" : "Balasan"}
                            </h5>
                            <button 
                                type="button" 
                                className="btn-close" 
                        ...
                        <div className="modal-body">
                            ...
                            <div className="detail-section">
                                <h6 className="section-title">
                                    {filterType === "question" ? "Pertanyaan yang Dilaporkan" : "Balasan yang Dilaporkan"}
                                </h6>
                                <div className="content-box">
                                    <div className="content-item">
                                        {filterType === "question" && (
                                            <>
                                                <p className="content-title"><strong>{selectedReport.question_title}</strong></p>
                                                <p className="content-author">Oleh: {selectedReport.question_user}</p>
                                            </>
                                        )}
                                        {filterType === "message" && (
                                            <>
                                                <p className="content-text">{selectedReport.message_content}</p>
                                                <p className="content-author">Oleh: {selectedReport.message_user}</p>
                                            </>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="detail-section">
                                ...
                                <div className="form-group">
                                    ...
                                    <small className="text-muted">
                                        Catatan ini akan dilihat oleh pengguna yang melaporkan
                                    </small>
                                </div>
                            </div>
                        </div>
