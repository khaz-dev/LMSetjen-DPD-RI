...
function Login() {
    ...
    const handlePopupMessage = async (event) => {
      ...
      if (event.data.type === "OAUTH_LOGIN_SUCCESS") {
        ...
        try {
          ...
          if (
            result.user.available_roles &&
            result.user.available_roles.length > 1
          ) {
            console.log(
              "👥 Multi-role user detected:",
              result.user.available_roles,
            );
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.log(
        "📱 Opening Google OAuth flow with client ID:",
  ...
  const handleGoogleResponse = async (response) => {
    ...
    try {
      setIsLoading(true);
      console.log(
        "✅ Google credential received, token length:",
      ...
      if (result.data && result.data.access) {
        ...
        // Check if user has multiple roles
        if (
          result.data.user.available_roles &&
          result.data.user.available_roles.length > 1
        ) {
          console.log(
            "👥 Multi-role user detected:",
            result.data.user.available_roles,
              ...
              <div className="card shadow border-0 login-card">
                <div className="card-body login-card-body">
                  ...
                  <div className="sso-login-container">
                    <h5
                      className="text-center mb-4 sso-login-title"
                      style={{
                        fontSize: "0.95rem",
                        fontWeight: "600",
                        color: "#495057",
                    ...
                    <div className="mb-3">
                      ...
                      >
                        <small>
                          <i className="fas fa-info-circle me-1"></i>Khusus
                          Pegawai Setjen DPD RI
                        </small>
                      </p>
                    ...
                    <div className="alert alert-info mt-4" role="alert">
                      ...
                      <small>
                        Gunakan akun Google atau SSO untuk login ke sistem
                        pembelajaran ini.
                      </small>
                    </div>
